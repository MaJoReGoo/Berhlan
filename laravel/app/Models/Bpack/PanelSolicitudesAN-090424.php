...
class PanelSolicitudesAN extends Model
{
  ...
  {
    $sql = DB::table('bpac_movimientosan')->where('solicitud', $id)->where('estado', '3')->orWhere('estado', '10')->orderBy('fecha', 'DESC')->get();
    //$sql = DB::table('bpac_movimientosan')->where('solicitud', $id)->where('estado', '3')->orderBy('fecha', 'DESC')->get();
    return $sql;
  }
